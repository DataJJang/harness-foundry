...
`show_start_path.py`는 현재 저장소의 추천 coordination mode, blocker, workboard 상태를 읽고 지금 바로 할 3가지 액션만 보여주는 optional helper다. 현재 model tier를 알면 `--current-model-tier standard`처럼 함께 넘겨 경고를 바로 볼 수 있다.
