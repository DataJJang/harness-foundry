...
특히 작은 팀이 반복적으로 수행하는 `brownfield migration`, `전자정부프레임워크 기반 프로젝트`, `KRDS/UI·UX 준수형 공공 프로젝트`에 맞춰 점진적으로 특화하는 방향을 지향한다.
...
- 작은 팀이 반복적으로 수행하는 공공/준공공 SI 프로젝트
- 전자정부프레임워크 기반 신규 구축, 유지보수, 고도화
- KRDS, 접근성, 공공형 UI/UX 기준을 따라야 하는 웹 서비스
- 최신 baseline보다 실제 운영 환경 제약이 더 강한 brownfield migration
...
- version provenance: [`source/docs/ai/governance/version-provenance.md`](./source/docs/ai/governance/version-provenance.md)
...
- `.agent-base/generation-manifest.json`
  - 생성 시점 foundry version, commit, template provenance 포함
