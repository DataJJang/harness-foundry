...
공공/전자정부/KRDS 성격이 강하면 여기에 [`org-specific/egov-public-sector-guide.md`](./org-specific/egov-public-sector-guide.md)도 같이 읽는 편이 좋다.
...
운영 환경이 먼저 고정된 프로젝트라면 `constraintMode`를 `fixed-target` 또는 `legacy-maintenance`로 두고, baseline보다 실제 target 환경을 먼저 spec에 기록한다.

전자정부프레임워크 기반 신규 구축이나 고도화는 아래처럼 시작하는 편이 안전하다.

- UI가 있으면 `web-app` 또는 `mockup-local`
- 업무/API는 `backend-service`
- 정기처리나 집계는 `batch-worker`
- 기관 간 연계 수신은 `receiver-integration`
- 별도 family를 새로 만들기보다 공통 family + 공공 특화 규칙 조합으로 푼다
...
가장 얇은 시작점이 필요하면 생성된 저장소에서 먼저 `python3 scripts/show_start_path.py`를 실행한다. 이 명령은 현재 repo state를 읽고 지금 바로 할 3가지 액션만 보여준다.
현재 tier를 알면 `--current-model-tier`를, 모델명만 알면 `--current-model-name`과 `.agent-base/model-tier-map.json`을 같이 넘겨 warning을 먼저 볼 수 있다.
