...
- If the tool only exposes a model name, use `.agent-base/model-tier-map.json` with `python3 scripts/show_start_path.py --current-model-name <model-slug> --model-tier-map-path <path>`
- Generation provenance for generated repos: `.agent-base/generation-manifest.json` records foundry version, template version, commit, and generation time
...
- Public-sector/eGov work:
  - keep the existing project families, and use `organizationProfile = egov-public-sector` to turn on compatibility, parity, KRDS, and rollout overlays
- Runtime roles:
...
- Technology and policy:
  ...
  - [`docs/ai/governance/version-provenance.md`](./docs/ai/governance/version-provenance.md)
  - [`docs/ai/tools/model-tier-mapping.md`](./docs/ai/tools/model-tier-mapping.md)
