...
| 조직/도메인 profile | `organizationProfile` | `none`, `egov-public-sector` | 추천 기본값 있음 |
| 런타임 역할 | `runtimeRoles` | `frontend`, `api`, `batch`, `receiver`, `client`, `tooling`, `worker` 중 하나 이상 | 초기 확정 필수 |
| 프론트 전달 정책 | `frontendArchitecturePolicy` | `jsp-mvc`, `separated-frontend-api`, `mpa-plus-ajax`, `spa`, `undecided`, `not-applicable` | 조건부 필수 |
| 운영 제약 모드 | `constraintMode` | `recommended-baseline`, `fixed-target`, `legacy-maintenance` | 추천 기본값 있음 |
| 고정 운영 제약 | `hardConstraints` | 운영 환경, OS 버전, 런타임/프레임워크 허용 범위, container 허용 여부 | 조건부 필수 |
| 공공 웹 제약 | `publicWebConstraints` | SEO, MPA/AJAX 정책, 크로스브라우징, 모바일 parity, 허용 frontend framework | 조건부 필수 |
...
- `constraintMode`: `recommended-baseline`
...
다만 `organizationProfile = egov-public-sector`이고 family가 `web-app`, `backend-service`, `batch-worker`면 quick-start도 공공 신규 구축 기본값을 우선 추천할 수 있다.

- `web-app`
  - 먼저 `frontendArchitecturePolicy`를 확인한다
  - `jsp-mvc`, `mpa-plus-ajax`면 `Java 17 + Maven + eGovFrame 4.3 JSP/Spring MVC`
  - `separated-frontend-api`면 `TypeScript + React + npm`
  - `undecided`면 quick-start를 유지하지 않고 `guided-review`로 올린다
- `backend-service`
  - `Java 17 + Maven + eGovFrame 4.3 REST + MyBatis`
- `batch-worker`
  - `Java 17 + Maven + eGovFrame 4.3 Batch + MyBatis`

`constraintMode`를 `fixed-target` 또는 `legacy-maintenance`로 바꾸면 quick-start는 기본적으로 `guided-review`로 승격한다. 이 경우 bootstrap CLI는 최소한 아래 제약을 먼저 확인한다.
...
- 허용 프레임워크 버전/정책
- container 허용 여부
- 현재 baseline scaffold 제외 여부

이 제약은 최신 baseline보다 우선한다. 즉, `Node 22`나 `Java 17` 같은 family baseline이 있더라도 실제 target 환경이 `Node 18`, `Java 11`, 구형 Linux, container 불가 같은 조건이면 그 고정 제약을 먼저 spec에 기록하고 scaffold 지원 수준을 낮출 수 있어야 한다.

즉, 최종 spec에는 값이 모두 남지만, 일반적인 quick-start에서는 위험도가 높은 항목을 보수적인 기본값으로 먼저 채우고 이후 refinement에서 확장하는 흐름을 기본으로 둔다.

`organizationProfile`이 `egov-public-sector`면 family는 그대로 두고 공공/전자정부 overlay만 활성화한다. 즉, 별도 family를 추가하는 대신 아래가 함께 따라온다.

- `docs/ai/org-specific/egov-public-sector-guide.md`
- `prompts/org-specific/egov-public-sector/*`
- 공공 UI/접근성 checklist와 prompt
- refinement의 `public-sector-profile` 모듈
- `frontendArchitecturePolicy`, `publicWebConstraints` 같은 공공 웹 의사결정 필드

## 2-1-1. 제약 우선순위

생성기와 bootstrap 인터뷰는 아래 우선순위를 따른다.

1. `hardConstraints`
2. 호환성 규칙과 지원 범위
3. project family baseline

즉 baseline은 기본 추천일 뿐이고, 고정 운영 제약을 덮어쓰면 안 된다. 현재 runnable scaffold가 그 제약을 만족하지 못하면 생성기는 `docs-only`로 강등하고 이유를 `.agent-base/generation-manifest.json`, root `README.md`, `TODO_UNSUPPORTED_SCAFFOLD.md`에 남긴다.
...
```json
{
  "repositoryName": "sample-legacy-backend",
  "projectName": "Sample Legacy Backend",
  "projectPurpose": "Maintain a backend service that must still run on a locked legacy server.",
  "projectFamily": "backend-service",
  "projectNature": "internal-tool",
  "organizationProfile": "none",
  "constraintMode": "fixed-target",
  "hardConstraints": {
    "operatingEnvironment": "2010-era on-prem server",
    "operatingSystemVersion": "legacy Linux with old glibc",
    "runtimeVersionPolicy": "Java 11 only",
    "frameworkVersionPolicy": "Spring Boot 2.7.x only",
    "containerAllowed": false,
    "blockCurrentBaselineScaffold": true,
    "notes": [
      "Upgrade path is deferred until infra replacement",
      "New scaffold defaults must not overwrite startup scripts"
    ]
  },
  "repositoryMode": "single-repo",
  "targetUsers": ["internal-operators"],
  ...
  "language": "Java",
  "runtimeVersion": "11",
  "framework": "Spring Boot 2.7.x (legacy)",
  "buildTool": "Gradle",
  "testTool": "Gradle test",
  "datastore": "없음",
  "cache": "없음",
  ...
  "targetOs": ["Linux"],
  "securityProfile": "JWT",
  "packageName": "com.example.legacybackend",
  "targetEnvironments": ["local", "dev"],
  "externalIntegrations": ["internal-api"],
  "baseDocumentSet": ["README", "build-guide", "test-plan", "impact-analysis"],
  "exceptions": [
    "constraintMode is `fixed-target` and hard constraints override the default family baseline",
    "runtimeVersion uses `11` instead of family default `17`",
    "framework uses `Spring Boot 2.7.x (legacy)` instead of family default `Spring Boot 3.5.x`",
    "deploymentType uses `VM` instead of family default `container`"
  ]
}
```
...
`generation-manifest.json`에는 scaffold 선택 결과와 함께 foundry version, template version, commit provenance가 들어간다.
또한 `organizationProfile`이 활성화돼 있으면 어떤 조직/도메인 overlay 기준으로 생성되었는지도 같이 남긴다.
